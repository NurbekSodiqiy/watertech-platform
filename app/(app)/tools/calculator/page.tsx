...
// Real product names, for the free-text-with-suggestions field below — this
// is display-only (what shows on the "mijozga yuborish" summary), it never
// feeds the arithmetic. Deduped since a few names repeat across variants
// (e.g. "Переходник" for both the ppr and kanalizatsiya lines).
const PRODUCT_NAMES = Array.from(new Set(products.map((p) => p.name_ru)));
...
const ALL_PACKAGES: Package[] = packageGroups.flatMap((g) => g.packages);
...
export default function CalculatorPage() {
  ...
  const selectedPackage = ALL_PACKAGES.find((p) => p.id === packageId) ?? null;
